...
function EWD_UASP_Get_Events($Selected_Location, $Selected_Service, $Selected_Service_Provider, $Start_Date, $End_Date) {
	...
	foreach ($Days as $Date => $Day) {
		...
		foreach ($Service_Providers as $Service_Provider) {
			...
			$Status_Changes[] = array('Time' => strtotime($Date . get_post_meta($Service_Provider->ID, $Day . ' Finish', true)), 'Event' => 'Shift', 'Status' => 'Unavailable');
			...
			foreach ($Appointments as $Appointment) {
				$Start_Time = strtotime($Appointment->Appointment_Start);
			...
			$Exceptions = $wpdb->get_results($wpdb->prepare("SELECT * FROM $ewd_usap_exceptions_table_name WHERE (Service_Provider_Post_ID=%d OR Location_Post_ID=%d) AND (Appointment_Start LIKE %s OR Exception_End LIKE %s)", $Service_Provider->ID, $Selected_Location, $Date . "%", $Date . "%"));
...
function EWD_UASP_Get_Appointments_Times($Selected_Location, $Selected_Service, $Selected_Service_Provider, $Selected_Date, $Appointment_ID = "") {
	...
	foreach ($Service_Providers as $Service_Provider) {
		...
		$Start_Time_Number = ConvertTimeToNumber($Start);
		$Finish_Time_Number = ConvertTimeToNumber($Finish);
		...
		foreach ($Appointments as $Appointment) {
			...
			$Appointment_Start_Time_Number = ConvertTimeToNumber($Appointment_Start_Time);
			$Appointment_End_Time_Number = ConvertTimeToNumber($Appointment_End_Time);

			foreach ($Appointment_Time_Numbers as $key => $Appt_Time) {
				if ((($Appt_Time + $Service_Duration_Hours) > $Appointment_Start_Time_Number) and ($Appt_Time < $Appointment_End_Time_Number)) {
		...
		if (count($Appointment_Time_Numbers) > 0) {
			...
			foreach ($Appointment_Time_Numbers as $Appointment_Time_Number) {
				...
				$Appointment_Start_Time = ConvertNumberToTime($Appointment_Time_Number);
				if ($Appointment_Start_Time == $Selected_Appointment_Time) {$Selected_Appointment_Class = "ewd-uasp-selected-appointment-time";}
				$ReturnString .= "<div class='ewd-uasp-das-appointment-listing " . $Selected_Appointment_Class . "'>";
				$ReturnString .= "<a class='ewd-uasp-das-appointment-link' onclick='SetAppointmentTime(this, \"". $Appointment_Start_Time . "\", \"" . $Service_Provider->ID . "\");'>";
				$ReturnString .= $Appointment_Start_Time;
				$ReturnString .= "</a>";
				$ReturnString .= "</div>";
			}
			$ReturnString .= "</div>";
...
function ConvertTimeToNumber($Time, $Format = "24") {
	if ($Format == "24") {
...
function ConvertNumberToTime($Time, $Format = "24") {
	if ($Format == "24") {
		$Hours = floor($Time);

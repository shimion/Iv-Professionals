...
function EWD_UASP_Add_Edit_Appointment() {
	...
	if (!isset($error)) {
		...
		if ($_POST['action'] == "Add_Appointment" or $_POST['action'] == "EWD_UASP_AddAppointment") {
			...
			if ($wpdb->insert_id and $Client_Email_Details != -1 and $_POST['action'] == "EWD_UASP_AddAppointment") {EWD_UASP_Send_Appointment_Notification("Client", $Client_Email_Details);}
			if ($wpdb->insert_id and $Provider_Email_Notification != -1 and $_POST['action'] == "EWD_UASP_AddAppointment") {EWD_UASP_Send_Appointment_Notification("Provider", $Provider_Email_Notification);}
			if ($wpdb->insert_id and $Admin_Email_Notification != -1 and $_POST['action'] == "EWD_UASP_AddAppointment") {EWD_UASP_Send_Appointment_Notification("Admin", $Admin_Email_Notification);}
...
function EWD_UASP_Send_Appointment_Notification($Recipient, $Email_ID) {
	global $wpdb;
	...
	$Appointment = $wpdb->get_row($wpdb->prepare("SELECT * FROM $ewd_usap_appointments_table_name WHERE Appointment_ID=%d", $wpdb->insert_id));
	...
	else {$Recipient_Email = get_option( 'admin_email' );}
